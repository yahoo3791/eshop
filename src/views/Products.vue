...
<loading v-model:active="isLoading">
  ...
  <div class="bg-dark pt-5">
    <div class="container-fluid pt-5">
      <div class="row">
          ...
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb p-2 breadcrumb-style tracking-wider">
              ...
              <li class="breadcrumb-item breadcrumb-item-none"
                aria-current="page">{{this.clickText}}
              </li>
            </ol>
          </nav>
        </div>
        ...
        :class="{'d-none': filterBar}">
          ...
          @click.prevent="productsFilter($event.target.innerText)"
          @keydown="productsFilter($event.target.innerText)">
            <a href="#"
            :class="{' products-active ' : this.clickText === '全部雜誌'}"
            class="list-group-item
            list-group-item-action border"
            aria-current="true">全部雜誌</a>
            <a href="#"
            :class="{' products-active ' : this.clickText === '時尚'}"
            class="list-group-item
            list-group-item-action
            border border-top-0">時尚</a>
            <a href="#"
            :class="{' products-active ' : this.clickText === '藝術'}"
            class="list-group-item
            list-group-item-action border border-top-0">藝術</a>
            <a href="#"
            :class="{' products-active ' : this.clickText === '旅遊'}"
            class="list-group-item
            list-group-item-action border border-top-0">旅遊</a>
            <a href="#"
            :class="{' products-active ' : this.clickText === '汽車'}"
            class="list-group-item
          ...
          justify-content-md-center align-items-center mt-3">
            <label for="sort"
            class="text-white tracking-wide font-medium pe-2 pb-md-1 d-inline-block">顯示方法
        ...
        <div class="col-12 col-md-9">
          <div class="d-md-none text-end">
            <w-switch
            v-model="filterBar"
            class="ma4 w-switch--wide d-inline-block"
            color="grey-dark2">
              <template #track><strong>{{ filterBar ? 'on' : 'off' }}</strong></template>
            </w-switch>
            <div class="py-2 text-white d-inline-block"
            @click="filterBar = !filterBar"
            @keydown="filterBar = !filterBar"
            :class="{'d-none': filterBar}">關閉所有篩選條件欄</div>
            <div class="py-2 text-white d-inline-block"
            @click="filterBar = !filterBar"
            @keydown="filterBar = !filterBar"
            :class="{'d-none': !filterBar}">開啟所有篩選條件欄</div>
          </div>
          <div class="row">
    ...
    ref="scrollTop"
    class="scrollTop-container position-fixed end-0 text-center">
      <div
